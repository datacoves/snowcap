...
def test_resource_pointer_rendering():
    ...
    network_rule = res.NetworkRule(
        name="SNOWCAP_TEST_NETWORK_RULE",
        type="IPV4",
    ...
    network_policy = res.NetworkPolicy(
        name="SNOWCAP_TEST_NETWORK_POLICY",
    ...
    assert (
        rendered
        == "CREATE NETWORK POLICY SNOWCAP_TEST_NETWORK_POLICY ALLOWED_NETWORK_RULE_LIST = (DB.SCH.SNOWCAP_TEST_NETWORK_RULE)"
    )
